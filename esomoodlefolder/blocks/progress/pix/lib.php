...
function block_progress_bar($modules, $config, $events, $userid, $instance, $attempts, $course, $simple = false) {
	...
	if (isset($config->showpercentage) && $config->showpercentage == 1) {
        ...
        $cellcontent = get_string('progress', 'block_progress').': '.$progress.'%';
		...
		//$grades = grade_get_course_grades($course, $userid);
		//$grade = $grades->grades[$userid];
		$cellcontent = 'Grade: '.round($grade->finalgrade, 2).'%';
		...
		$dedicationtime = $dm->get_user_dedication($userrecord, false);
		$cellcontent = 'In Class: '.$dedicationtime.' secs';
