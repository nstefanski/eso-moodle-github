...
class block_classmates extends block_base {
    ...
    function get_content() {
        ...
        $numshowusers = 30; //Class size default tk
        ...
        if ($currentgroup !== NULL) {
			...
			$groupjoin = "JOIN {groups_members} gm ON u.id = gm.userid AND gm.groupid = :currentgroup ";
        ...
        if ($this->page->course->id == SITEID or $this->page->context->contextlevel < CONTEXT_COURSE) {  // Site-level
			...
			$rolesql = "SELECT GROUP_CONCAT(r.shortname)
           ...
           $csql = "SELECT COUNT(u.id)
                     ...
                     WHERE u.deleted = 0
					   AND u.suspended = 0
					   AND ($rolesql) LIKE '%student%' ";

            $params['courseid'] = $this->page->course->id;
        }

        // Verify if we can see the list of users, if not just print empty
        ...
        if ($usercount < $numshowusers  && !has_capability('moodle/user:viewalldetails', $this->page->context)) { //tk added 3-18-16
            $usercount = '';
        } else {
            $usercount = 'Total Classmates: '.$usercount;
			if (has_capability('moodle/user:viewalldetails', $this->page->context)) { //tk added 3-24-16
				$usercount .= '<br><a href="'.$CFG->wwwroot.'/user/index.php?id='.$this->page->course->id.'">See All Classmates</a>';
			}
        }
