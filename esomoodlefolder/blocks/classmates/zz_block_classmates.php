        ...
        if (!empty($users)) {
            ...
            foreach ($users as $user) {
                ...
                if (isguestuser($user)) {
					...
					$this->content->text .= $OUTPUT->user_picture($user, array('size'=>35, 'alttext'=>false, 'link'=>false, 'class'=>$user_pic_class)) .$user->fullname.' '//.$employee_role->data
						.'</a></div>';
                }
				...
				if ($canshownotes and !isguestuser($user)) { //tk added 8-7
					...
					$anchortagcontents = '{'.$notecount.'}';
					$anchortag = '<a href="'.$CFG->wwwroot.'/notes/index.php?user='.$user->id.'" title="notes">'.$anchortagcontents .'</a>';
					$this->content->text .= '<div class="classmates-notes" style="float: right;">'.$anchortag.'</div>';
				}
